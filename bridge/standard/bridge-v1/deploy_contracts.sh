...
DEPLOYER_KEYSTORE_DIR=${DEPLOYER_KEYSTORE_DIR:-"../../../contracts/deployer_keystore"}
DEPLOYER_KEYSTORE_PASSWORD=${DEPLOYER_KEYSTORE_PASSWORD:-"pwd"}
...
SETTLEMENT_DEPLOYER_ADDR=$("$CAST_BIN_PATH" wallet address --keystore "$DEPLOYER_KEYSTORE_DIR"/* --password "$DEPLOYER_KEYSTORE_PASSWORD")
...
"$CAST_BIN_PATH" send \
    ...
    --keystore "$DEPLOYER_KEYSTORE_DIR"/* \
    --password "$DEPLOYER_KEYSTORE_PASSWORD" \
    "$RELAYER_ADDR" \
    --value 100ether
...
RELAYER_ADDR="$RELAYER_ADDR" $FORGE_BIN_PATH script \
    ...
    --rpc-url "$STANDARD_BRIDGE_RELAYER_SETTLEMENT_RPC_URL" \
    --keystores "$DEPLOYER_KEYSTORE_DIR" \
    --password "$DEPLOYER_KEYSTORE_PASSWORD" \
    --broadcast \
