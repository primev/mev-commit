    ...
    task "bidder-node" {
      ...
      template {
        data = <<-EOH
          ...
          KEYSTOREGEN_LOG_TAGS="service:mev-commit-bidder-node1"
          MEV_COMMIT_LOG_FMT="json"
          MEV_COMMIT_LOG_TAGS="service:mev-commit-bidder-node1"
          MEV_COMMIT_KEYSTORE_PATH = "/local/data-{{env "NOMAD_ALLOC_INDEX"}}/keystore"
          MEV_COMMIT_KEYSTORE_PASSWORD = "{{ with secret "secret/data/mev-commit" }}{{ .Data.data.bidder1_keystore_password }}{{ end }}"
        {{ range nomadService "mev-commit-geth-bootnode1" }}
          {{- if contains "http" .Tags }}
          MEV_COMMIT_SETTLEMENT_RPC_ENDPOINT="http://{{ .Address }}:{{ .Port }}"
          {{ end }}
        {{ end }}
          MEV_COMMIT_PEER_TYPE = "bidder"
        ...
        {{- range nomadService "datadog-agent-logs-collector" }}
          {{- if contains "mev-commit-bidder-node1-sink" .Tags }}
