job "mev-commit-provider-node1-funder" {
  datacenters = ["dc1"]
  type = "batch"

  group "mev-commit-provider-node-funder-group" {
    count = 1

    task "provider-node-funder" {
      driver = "exec"
      ...
      template {
        ...
        {{- range nomadService "mev-commit-provider-node1" }}
          ...
          while : ; do
              ...
              if [ "${STATUS_CODE}" -eq 200 ]; then
                  ...
                  if [ "${ELAPSED_TIME}" -ge "${TIMEOUT}" ]; then
                      echo "Timeout reached; the provider node is not up."
                      exit 1
                  fi
                  sleep 1
              fi
          ...
          if ! curl -s -X POST https://{{ .Address }}:{{ .Port }}/v1/provider/register_stake/10000000000000000000 > /dev/null; then
