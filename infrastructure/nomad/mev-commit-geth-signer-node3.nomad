job "mev-commit-geth-signer-node3" {
  datacenters = ["dc1"]
  ...
  group "signer-node-group" {
    ...
    task "signer-node" {
      ...
      artifact {
        source = "https://raw.githubusercontent.com/primevprotocol/mev-commit-geth/${NOMAD_META_MEV_COMMIT_GETH_VERSION}/geth-poa/signer-node3/keystore/UTC--2024-02-25T14-43-45.403628523Z--f6ba5bca9b489de3a4ac3a57823d5d97d608feb9"
      ...
      template {
        data = <<-EOH
          ...
          GETH_LOG_TAGS="service:mev-commit-geth-signer-node3"
          GETH_NODE_TYPE = "signer"
          GENESIS_L1_PATH = "local/genesis.json"
          GETH_DATA_DIR = "local/data-{{env "NOMAD_ALLOC_INDEX"}}"
          MEV_COMMIT_GETH_PASSWORD = "{{ with secret "secret/data/mev-commit" }}{{ .Data.data.geth_signer3_keystore_password }}{{ end }}"
