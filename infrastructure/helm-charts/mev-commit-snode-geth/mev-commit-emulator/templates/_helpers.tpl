...
{{- if not (or (eq .Values.job.type "bidder") (eq .Values.job.type "provider")) }}
{{- fail "job.type must be either 'bidder' or 'provider'" }}
{{- end }}
{{- if eq .Values.job.type "bidder" }}
{{- if not .Values.job.rpcAddr }}
{{- fail "job.rpcAddr is required when job.type is 'bidder'" }}
{{- end }}
{{- end }}
{{- if eq .Values.job.type "provider" }}
{{- if not .Values.job.relay }}
{{- fail "job.relay is required when job.type is 'provider'" }}
{{- end }}
{{- end }}
{{- end }}
