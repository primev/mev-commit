...
func main() {
	...
	hoursFlag := flag.Int("hours", 24*80, "Number of hours to look back")
...
func runMonitoring(ctx context.Context, hours int, verbose bool, checkRelays bool, maxWorkers int, excelEnabled bool, excelFile string) error {
	...
	if excelEnabled && len(optedInValidators) > 0 {
		if err := exportToExcelWithAppend(optedInValidators, excelFile); err != nil {
...
func exportToExcelWithAppend(validators []ValidationDuty, filePath string) error {
	var f *excelize.File
	var err error
	...
	if fileExists {
		...
		if err != nil {
			return fmt.Errorf("error opening existing Excel file: %v", err)
		}

		// Get the default sheet name
		sheetName := f.GetSheetName(0)

		// Count existing rows to know where to append
		rows, err := f.GetRows(sheetName)
		if err != nil {
			existingRows = 1 // Start at row 2 (after headers) if we can't determine
		} else {
			existingRows = len(rows)
			if existingRows == 0 {
				existingRows = 1 // Add headers if file exists but is empty
			}
		}
	} else {
		// Create a new file if it doesn't exist
		f = excelize.NewFile()
		existingRows = 1 // Start at row 2 (after headers)
	...
	if err != nil {
		log.Warn().Err(err).Msg("Could not create hyperlink style")
	}
	...
	// Add data for each validator
	for i, validator := range validators {
		row := existingRows + i + 1 // Start after existing rows
		...
		f.SetCellValue(sheetName, fmt.Sprintf("F%d", row), validator.EtherscanURL)
		f.SetCellValue(sheetName, fmt.Sprintf("G%d", row), validator.BeaconchainURL)
		f.SetCellValue(sheetName, fmt.Sprintf("H%d", row), currentTime) // Add timestamp
		...
		// Apply hyperlink style to URL columns
		if hyperlinkStyle != 0 {
			f.SetCellStyle(sheetName, fmt.Sprintf("F%d", row), fmt.Sprintf("G%d", row), hyperlinkStyle)
		}
	}
