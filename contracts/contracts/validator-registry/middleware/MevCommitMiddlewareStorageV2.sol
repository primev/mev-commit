...
abstract contract MevCommitMiddlewareStorageV2 {
    ...
    mapping(bytes blsPubkey => IMevCommitMiddlewareV2.ValidatorRecord) public validatorRecords;

    /// @notice Mapping of an operator's address to its operator record.
    mapping(address operatorAddress => IMevCommitMiddlewareV2.OperatorRecord) public operatorRecords;

    /// @notice Mapping of a vault's address to its vault record.
    mapping(address vaultAddress => IMevCommitMiddlewareV2.VaultRecord) public vaultRecords;

    /// @notice Mapping of a vault and operator to block number to slash record.
    mapping(address vault => mapping(address operator => mapping(uint256 blockNumber => IMevCommitMiddlewareV2.SlashRecord))) public slashRecords;
