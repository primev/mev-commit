...
func (_L1gateway *L1gatewayTransactor) SetRelayer(opts *bind.TransactOpts, _relayer common.Address) (*types.Transaction, error) {
	return _L1gateway.contract.Transact(opts, "setRelayer", _relayer)
}

// SetRelayer is a paid mutator transaction binding the contract method 0x6548e9bc.
//
// Solidity: function setRelayer(address _relayer) returns()
func (_L1gateway *L1gatewaySession) SetRelayer(_relayer common.Address) (*types.Transaction, error) {
	return _L1gateway.Contract.SetRelayer(&_L1gateway.TransactOpts, _relayer)
}

// SetRelayer is a paid mutator transaction binding the contract method 0x6548e9bc.
//
// Solidity: function setRelayer(address _relayer) returns()
func (_L1gateway *L1gatewayTransactorSession) SetRelayer(_relayer common.Address) (*types.Transaction, error) {
	return _L1gateway.Contract.SetRelayer(&_L1gateway.TransactOpts, _relayer)
}
...
// L1gatewayRelayerSetIterator is returned from FilterRelayerSet and is used to iterate over the raw logs and unpacked data for RelayerSet events raised by the L1gateway contract.
type L1gatewayRelayerSetIterator struct {
	Event *L1gatewayRelayerSet // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *L1gatewayRelayerSetIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(L1gatewayRelayerSet)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *L1gatewayRelayerSetIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *L1gatewayRelayerSetIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// L1gatewayRelayerSet represents a RelayerSet event raised by the L1gateway contract.
type L1gatewayRelayerSet struct {
...
func (_L1gateway *L1gatewayFilterer) FilterRelayerSet(opts *bind.FilterOpts, relayer []common.Address) (*L1gatewayRelayerSetIterator, error) {
	...
	logs, sub, err := _L1gateway.contract.FilterLogs(opts, "RelayerSet", relayerRule)
	if err != nil {
		return nil, err
	}
	return &L1gatewayRelayerSetIterator{contract: _L1gateway.contract, event: "RelayerSet", logs: logs, sub: sub}, nil
}
...
//
// Solidity: event RelayerSet(address indexed relayer)
func (_L1gateway *L1gatewayFilterer) WatchRelayerSet(opts *bind.WatchOpts, sink chan<- *L1gatewayRelayerSet, relayer []common.Address) (event.Subscription, error) {
	...
	logs, sub, err := _L1gateway.contract.WatchLogs(opts, "RelayerSet", relayerRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(L1gatewayRelayerSet)
				if err := _L1gateway.contract.UnpackLog(event, "RelayerSet", log); err != nil {
					return err
				}
				event.Raw = log
...
//
// Solidity: event RelayerSet(address indexed relayer)
func (_L1gateway *L1gatewayFilterer) ParseRelayerSet(log types.Log) (*L1gatewayRelayerSet, error) {
	event := new(L1gatewayRelayerSet)
	if err := _L1gateway.contract.UnpackLog(event, "RelayerSet", log); err != nil {
		return nil, err
	}
