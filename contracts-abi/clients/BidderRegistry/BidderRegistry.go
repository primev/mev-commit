...
// BidderregistryFundsAccumulatedForTreasuryIterator is returned from FilterFundsAccumulatedForTreasury and is used to iterate over the raw logs and unpacked data for FundsAccumulatedForTreasury events raised by the Bidderregistry contract.
type BidderregistryFundsAccumulatedForTreasuryIterator struct {
	Event *BidderregistryFundsAccumulatedForTreasury // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *BidderregistryFundsAccumulatedForTreasuryIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(BidderregistryFundsAccumulatedForTreasury)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *BidderregistryFundsAccumulatedForTreasuryIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *BidderregistryFundsAccumulatedForTreasuryIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// BidderregistryFundsAccumulatedForTreasury represents a FundsAccumulatedForTreasury event raised by the Bidderregistry contract.
type BidderregistryFundsAccumulatedForTreasury struct {
...
func (_Bidderregistry *BidderregistryFilterer) FilterFundsAccumulatedForTreasury(opts *bind.FilterOpts, commitmentDigest [][32]byte, bidder []common.Address, provider []common.Address) (*BidderregistryFundsAccumulatedForTreasuryIterator, error) {
	...
	logs, sub, err := _Bidderregistry.contract.FilterLogs(opts, "FundsAccumulatedForTreasury", commitmentDigestRule, bidderRule, providerRule)
	if err != nil {
		return nil, err
	}
	return &BidderregistryFundsAccumulatedForTreasuryIterator{contract: _Bidderregistry.contract, event: "FundsAccumulatedForTreasury", logs: logs, sub: sub}, nil
}
...
//
// Solidity: event FundsAccumulatedForTreasury(bytes32 indexed commitmentDigest, uint256 amount, uint256 totalAccumulated, address recipient, address indexed bidder, address indexed provider)
func (_Bidderregistry *BidderregistryFilterer) WatchFundsAccumulatedForTreasury(opts *bind.WatchOpts, sink chan<- *BidderregistryFundsAccumulatedForTreasury, commitmentDigest [][32]byte, bidder []common.Address, provider []common.Address) (event.Subscription, error) {
	...
	logs, sub, err := _Bidderregistry.contract.WatchLogs(opts, "FundsAccumulatedForTreasury", commitmentDigestRule, bidderRule, providerRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(BidderregistryFundsAccumulatedForTreasury)
				if err := _Bidderregistry.contract.UnpackLog(event, "FundsAccumulatedForTreasury", log); err != nil {
					return err
				}
				event.Raw = log
...
//
// Solidity: event FundsAccumulatedForTreasury(bytes32 indexed commitmentDigest, uint256 amount, uint256 totalAccumulated, address recipient, address indexed bidder, address indexed provider)
func (_Bidderregistry *BidderregistryFilterer) ParseFundsAccumulatedForTreasury(log types.Log) (*BidderregistryFundsAccumulatedForTreasury, error) {
	event := new(BidderregistryFundsAccumulatedForTreasury)
	if err := _Bidderregistry.contract.UnpackLog(event, "FundsAccumulatedForTreasury", log); err != nil {
		return nil, err
	}
