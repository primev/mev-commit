...
func (h HeadType) Verify() error {
	if !allHeadTypes[h] {
		return fmt.Errorf("invalid head type, head: %s", h)
	}
...
func Dial(chainName string, url string) (Wrapper, error) {
	...
	if err != nil {
		return Wrapper{}, fmt.Errorf("failed to dial, chain: %s , url: %s, err: %w", url, chainName, err)
	}
...
func (w Wrapper) SetHead(ctx context.Context, height uint64) error {
	...
	if err != nil {
		incError(w.chain, endpoint)
		return fmt.Errorf("failed to set head, err: %w", err)
	}
...
func (w Wrapper) PeerCount(ctx context.Context) (uint64, error) {
	...
	if err != nil {
		incError(w.chain, endpoint)
		return 0, fmt.Errorf("failed to get peer count, err: %w", err)
	}
