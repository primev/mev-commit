...
func run(_ context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("load package, err: %w", err)
	}
...
func parseImports(pkg *packages.Package) ([]string, error) {
	...
	for _, file := range pkg.Syntax {
		for _, imprt := range file.Imports {
			...
			if err != nil {
				return nil, fmt.Errorf("printf, err: %w", err)
			}
...
func writeTemplate(methods []Method, providers []string, imprts []string) error {
	...
	if err != nil {
		return fmt.Errorf("parse template, err: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("exec template, err: %w", err)
	}

	filename := "ethclient_gen.go"
	out, err := imports.Process(filename, b.Bytes(), nil)
	if err != nil {
		return fmt.Errorf("format, err: %w", err)
	}
			...
			for _, spec := range gendecl.Specs {
				...
				for _, method := range iface.Methods.List {
					...
					for _, param := range fnType.Params.List {
						...
						if err != nil {
							return nil, nil, fmt.Errorf("printf, err: %w", err)
						}
